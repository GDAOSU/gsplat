...
__global__ void rasterize_to_pixels_ortho_2dgs_fwd_kernel(
    ...
    for (uint32_t b = 0; b < num_batches; ++b) {
         ...
         *
         * 1. Compute uv directly from the ray transformation:
         *    u = p_x * M_u[0] + p_y * M_u[1] + M_u[2]
         *    v = p_x * M_v[0] + p_y * M_v[1] + M_v[2]
         * 
        ...
        for (uint32_t t = 0; (t < batch_size) && !done; ++t) {
            ...
            // const vec3 w_M = w_Ms_batch[t]; // SHOULD BE 0,0,1
...
void launch_rasterize_to_pixels_ortho_2dgs_fwd_kernel(
    ...
    if (cudaFuncSetAttribute(
            rasterize_to_pixels_ortho_2dgs_fwd_kernel<CDIM, float>,
