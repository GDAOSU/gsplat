...
def ortho_depth_to_points(depths: Tensor, camtoworlds: Tensor, Ks: Tensor) -> Tensor:
    ...
    # ray directions in world coordinates
    world_xyz = torch.einsum(
        "...ij,...hwj->...hwi", camtoworlds[..., :3, :3], camera_xyz
    ) + camtoworlds[..., None,None, :3, -1] # [..., H, W, 3]
    
    return world_xyz


def ortho_depth_to_normal(depths: Tensor, camtoworlds: Tensor, Ks: Tensor) -> Tensor:
    """Convert depth maps to surface normals
    ...
    Returns:
        normals: Surface normals in the world coordinate system [..., H, W, 3]
    """
    points = ortho_depth_to_points(depths, camtoworlds, Ks,)  # [..., H, W, 3]
    dx = torch.cat(
        [points[..., 2:, 1:-1, :] - points[..., :-2, 1:-1, :]], dim=-3
    )  # [..., H-2, W-2, 3]
    dy = torch.cat(
