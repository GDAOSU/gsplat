...
def test_ortho_projection_2dgs(test_data, batch_dims: Tuple[int, ...]):
    ...
    if False:
        v_means = torch.autograd.grad((means2d * v_means2d).sum(), (means,))
        _v_means = torch.autograd.grad((_means2d * v_means2d).sum(), (means,))
        print(v_means)
        print(_v_means)
        torch.testing.assert_close(v_means, _v_means, rtol=6e-2, atol=1e-3)
    else:
        ...
        print("-- cuda impl viewmats")
        print(v_viewmats)
        print("-- python impl viewmats")
        print(_v_viewmats)

        print("-- cuda impl quats")
        print(v_quats)
        print("-- python impl quats")
        print(_v_quats)

        print("-- cuda impl scales")
        print(v_scales)
        print("-- python impl scales")
        print(_v_scales)

        print("-- cuda impl means")
        print(v_means)
        print("-- python impl means")
        print(_v_means)
